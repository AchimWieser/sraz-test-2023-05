...
function AddAzureADAppCertCred {
	...
	try {
		...
		$appKeyCreds = Az.Resources\Get-AzADAppCredential -ObjectId $AppObjectID
		...
		Az.Resources\New-AzADAppCredential @keyCredArgs
...
function GetRequiredPermissions {
	...
	if ($ServicePrincipal) {
		...
		$sp = Az.Resources\Get-AzADServicePrincipal -Filter "DisplayName eq '$ApplicationDisplayName'" | Select-Object -First 1
	}
	$appid = $sp.AppId
...
function GrantApiResourceAccess {
	...
	"Run '$($PSCmdlet.MyInvocation.MyCommand)' with Parameters$($PSBoundParameters | Out-String)" | Write-SRLog -LogType Verbose -PassThru | Write-Verbose
	$aadApp = Az.Resources\Get-AzADApplication -ObjectId $AppObjectId
	...
	Az.Resources\Update-AzADApplication -ObjectId $AppObjectId -RequiredResourceAccess $requiredResourceAccesses
}
...
function WaitForAppRegistration {
	...
	while ($waitCount -lt $waitPeriod) {
		...
		try {
			$appRegistration = Az.Resources\Get-AzADApplication @reqArgs
		}
		catch {
			Write-Debug "$($_)"
			$appRegistration = $null
...
function WaitForServicePrincipal {
	...
	while ($waitCount -lt $waitPeriod) {
		...
		try {
			$sp = Az.Resources\Get-AzADServicePrincipal @reqArgs
		}
		catch {
			Write-Debug "$($_)"
			$sp = $null
	...
	foreach($preAuthorizedApp in $appRegistration.Api.PreAuthorizedApplication) {
		...
		if($preAuthorizedApp.AppId -eq $ClientAppId) {
			if($preAuthorizedApp.DelegatedPermissionId.Contains($oauth2Permission.Id)) {
				...
				Az.Resources\Update-AzADApplication -ObjectId $appRegistration.Id -Api $appRegistration.Api
				return
			}
		}
	}
...
function SetGroupMembershipClaims {
	...
	if([string]::IsNullOrEmpty($appRegistration.GroupMembershipClaim)) {
		...
		Az.Resources\Update-AzADApplication -ObjectId $appRegistration.Id -GroupMembershipClaim $GroupMembershipClaims
...
function SetApplicationLogo {
	...
	"Run '$($PSCmdlet.MyInvocation.MyCommand)' with Parameters$($PSBoundParameters | Out-String)" | Write-SRLog -LogType Verbose -PassThru | Write-Verbose
	$logo = $null
	if($AppType -eq 'Service') { # Logo Service
...
function ConfigureAADApplications {
	...
	process {
		...
		if ($null -eq $serviceAadApplication) {
			...
			$serviceAadApplication = Az.Resources\Get-AzADApplication -Filter "DisplayName eq '$appNameAPI'" | Select-Object -First 1
		}
		...
		if ($null -eq $serviceAadApplication) {
			...
			$serviceAadApplication = Az.Resources\New-AzADApplication @aadAppArgs
		...
		Az.Resources\Update-AzADApplication -ObjectId $serviceAadApplication.Id -IdentifierUris $serviceIdentifierUri | Write-SRLog -PassThru | Write-Output
		...
		if ($null -eq $serviceServicePrincipal) {
			$serviceServicePrincipal = Az.Resources\Get-AzADServicePrincipal -Filter "DisplayName eq '$appNameAPI'" | Select-Object -First 1
		}
		if ($null -eq $serviceServicePrincipal) {
			...
			$serviceServicePrincipal = Az.Resources\New-AzADServicePrincipal -ApplicationId $serviceAadApplication.AppId -Tag 'WindowsAzureActiveDirectoryIntegratedApp' -ErrorAction Stop
		...
		# if (-not $OnlyService.IsPresent) {
			...
			if ($null -eq $spaAadApplication) {
				...
				$spaAadApplication = Az.Resources\Get-AzADApplication -Filter "DisplayName eq '$appNameSPA'" | Select-Object -First 1
			}
			...
			if ($null -eq $spaAadApplication) {
				...
				$spaAadApplication = Az.Resources\New-AzADApplication @aadAppArgs
			...
			#-Oauth2AllowImplicitFlow $False -Homepage $backendUri
			Az.Resources\Update-AzADApplication -ObjectId $spaAadApplication.Id -IdentifierUris $spaIdentifierUri | Write-SRLog -PassThru | Write-Output
			...
			Az.Resources\Update-AzADApplication -ObjectId $spaAadApplication.Id -SPARedirectUri $myRedirectUris
			...
			if ($null -eq $spaServicePrincipal) {
				...
				$spaServicePrincipal = Az.Resources\Get-AzADServicePrincipal -Filter "DisplayName eq '$appNameSPA'" | Select-Object -First 1
			}
			if ($null -eq $spaServicePrincipal) {
				# create the service principal of the newly created application
				...
				$spaServicePrincipal = Az.Resources\New-AzADServicePrincipal -ApplicationId $spaAadApplication.AppId -Tag { WindowsAzureActiveDirectoryIntegratedApp }
			...
			Az.Resources\Update-AzADApplication -ObjectId $spaAadApplication.Id -RequiredResourceAccess $requiredResourcesAccess
			...
			Az.Resources\Update-AzADApplication -ObjectId $serviceAadApplication.Id -Api $api
			...
			$serviceServicePrincipal = Az.Resources\Get-AzADServicePrincipal -ObjectId $serviceServicePrincipal.Id
